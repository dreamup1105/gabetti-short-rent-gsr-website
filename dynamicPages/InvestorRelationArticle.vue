<template>
  <GsrBoxedLayout>
    <GsrBlocksList :blocks="blocks.filter((block) => !['body', 'documents','table','image-body'].includes(block.key))"/>
    <!-- Main Container -->
    <div class="gsr-columns-wrapper">
      <!-- LEFT COLUMN -->
      <div class="gsr-left-column">
        <GsrBlocksList
          :blocks="blocks.filter((block) => ['body', 'documents','table','image-body'].includes(block.key))"/>
        <div class="grid grid-cols-1 gap-4" v-if="blocks.filter((block) => ['articles'].includes(block.key)).length">
          <div v-for="item in blocks.filter((block) => ['articles'].includes(block.key))[0].items" :key="item.id">
            <nuxt-link :to="localePath(`/${item.content.permalink.value}`)" v-if="item.content.permalink">
              <GsrCmsArticleCard :data="item" class="cursor-pointer" />
            </nuxt-link>
            <GsrCmsArticleCard v-else :data="item" />
          </div>
        </div>
...
export default {
  ...
  async asyncData({route, app, query, error}) {
    ...
    try {
      ...
      articles = (await app.$services.cms.getArticlesByType('investor-relation', routeData.locale)).data
    } catch (e) {
      console.error(e)
    }
