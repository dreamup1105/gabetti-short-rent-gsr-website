    ...
    <div class="flex flex-col mt-12 max-w-2xl m-auto pb-12">
      <div class="flex flex-col space-y-4 items-start text-left">
        <h1 class="text-2xl md:text-4xl font-bold">{{
            status === 'initial' ? $t('common.contact_request_title') : $t('common.contact_request_success_title')
          }}</h1>
        <h2 class="text-xl md:text-2xl font-normal">{{
            status === 'initial' ? $t('common.contact_request_subtitle') : $t('common.contact_request_success_subtitle')
          }}</h2>
      </div>
      <div class="mt-8 flex flex-col space-y-4">
        <div v-if="status === 'initial'" class="flex flex-col space-y-4">
          <GsrContactRequestForm v-model="form" :is-loading="isLoading" @submit="onSave"/>
        </div>
...
export default {
  ...
  components: {
    ...
    GsrContactRequestForm: () => import('@/components/forms/GsrContactRequestForm.vue'),
  ...
  methods: {
    ...
    async onSave() {
      this.isLoading = true
      try {
        await this.$services.contactRequest.store(this.form, this.$i18n.locale)
        this.$gtm.push({event: 'invioFormOk', form_name: 'contact-request'})
        this.status = 'success'
